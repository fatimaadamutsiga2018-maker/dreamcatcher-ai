...
-- All tables prefixed with claritypath_ to avoid conflicts

-- 1. Profiles Table
CREATE TABLE claritypath_profiles (
...
ALTER TABLE claritypath_profiles ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view own profile" ON claritypath_profiles FOR SELECT USING (auth.uid() = id);
CREATE POLICY "Users can update own profile" ON claritypath_profiles FOR UPDATE USING (auth.uid() = id);

-- 2. Assessment Sessions Table
CREATE TABLE claritypath_assessment_sessions (
...
CREATE INDEX idx_claritypath_assessment_user_id ON claritypath_assessment_sessions(user_id);
CREATE INDEX idx_claritypath_assessment_created_at ON claritypath_assessment_sessions(created_at DESC);

ALTER TABLE claritypath_assessment_sessions ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view own assessments" ON claritypath_assessment_sessions FOR SELECT USING (auth.uid() = user_id OR user_id IS NULL);
CREATE POLICY "Anyone can create assessment" ON claritypath_assessment_sessions FOR INSERT WITH CHECK (true);

-- 3. Hexagram Readings Table
CREATE TABLE claritypath_hexagram_readings (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID REFERENCES auth.users(id) ON DELETE SET NULL,
...
CREATE INDEX idx_claritypath_hexagram_user_id ON claritypath_hexagram_readings(user_id);
CREATE INDEX idx_claritypath_hexagram_created_at ON claritypath_hexagram_readings(created_at DESC);

ALTER TABLE claritypath_hexagram_readings ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view own readings" ON claritypath_hexagram_readings FOR SELECT USING (auth.uid() = user_id OR user_id IS NULL);
CREATE POLICY "Anyone can create reading" ON claritypath_hexagram_readings FOR INSERT WITH CHECK (true);

-- 4. Hexagram Templates Table
CREATE TABLE claritypath_hexagram_templates (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
INSERT INTO claritypath_hexagram_templates (hexagram_index, name_cn, name_en, element, core_theme, energy_state, action_suggestions, timing_advice, risk_warning) VALUES
